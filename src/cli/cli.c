...
/********************************************************************/
extern void console_getline_init(void);
extern char *console_getline(void);
...
void cliRegisterTable(char *name, char *help, int numberInTable, CliJte *table)
{
    ...
    {
        CLI_PRINT("Out of space in command table.\n");
        return;
    }

    if (cliFindCommand(name, &cliRootNode) != NULL)
    {
        CLI_PRINT("Table '%s' registered twice.\n", name);
        return;
    }
...
static void cliHelp(CliJte *jte)
{
    ...
    if (jte != &cliRootNode)
    {
        ...
        {
            CLI_PRINT("%-25s", jte->name);
        }
        else
#endif
        {
            CLI_PRINT("%-14s", jte->name);
        }
...
size_t dbgGetString(char *pbLine, unsigned int cbLine)
{
    ...
    if (line)
    {
        strncpy(pbLine, line, cbLine - 1);
        pbLine[cbLine - 1] = '\0'; // Ensure null termination
        return strlen(pbLine);
    }
#endif
    return 0;
